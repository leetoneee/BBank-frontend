...
function Edit() {
  const [inputData, setInputData] = useState({ MaPhanQuyen: '', MaChucNang: '', MaNhom: '' });
  ...
  useEffect(() => {
    if (transaction) {
      setInputData({
        MaPhanQuyen: transaction.MaPhanQuyen,
  ...
  function handleSubmit(event) {
    ...
    axios.post('http://localhost:3005/api/v1/group-role/update', inputData)
      .then(res => {
