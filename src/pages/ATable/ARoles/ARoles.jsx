...
const ARoles = () => {
    ...
    const handleDelete = (MaPhanQuyen) => {
        const conf = window.confirm("Do you want to delete?");
        if (conf) {
            axios.post('/group-role/delete', { MaPhanQuyen })
                .then(res => {
                ...
                <div className="h-auto flex flex-col min-h-screen  no-scrollbar">
                    ...
                    <table className="w-full text-left text-gray-500 table">
                        <thead className="text-[23px] text-gray-700 uppercase bg-gray-200 sticky top-[170px] z-10 ">
                            <tr>
                                <th className="p-4">Mã phân quyền</th>
                                <th>Mã chức năng</th>
                                <th>Mã nhóm</th>
                                <th>Action</th>
                            </tr>
                        ...
                        <tbody className=" text-orange-600 text-[22px]">
                            {
                                filteredData.map((d, i) => (
                                    <tr key={i} className="hover:bg-gray-400 border-b">
                                        <td className="p-3">{d.MaPhanQuyen}</td>
                                        <td>{d.MaChucNang}</td>
                                        <td>{d.MaNhom}</td>
                                        <td>
                                            <button
                                            ...
                                            <button onClick={() => handleDelete(d.MaPhanQuyen)} className="bg-red-500 hover:bg-red-700 text-white py-1 px-3 rounded text-lg ml-2">
                                                Xóa
                                            </button>
                                        </td>
                                    </tr>
