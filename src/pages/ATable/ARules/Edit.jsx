...
function Edit() {
  const [inputData, setInputData] = useState({ MaThamSo: '', Ten: '', GiaTri: '' });
  ...
  useEffect(() => {
    if (transaction) {
      setInputData({
        MaThamSo: transaction.MaThamSo,
        Ten: transaction.Ten,
        GiaTri: transaction.GiaTri
      });
    }
  }, [transaction]);
  ...
  function handleSubmit(event) {
    ...
    axios.post('/rule/update', inputData)
      .then(res => {
        ...
        <div className="w-2/5 border border-gray-300 p-5 bg-gray-100 text-[23px]">
          <form onSubmit={handleSubmit}>
            ...
            <div>
              <label>Giá trị:</label>
              <input
                value={inputData.GiaTri}
                className="border border-gray-300 mt-1 block w-full rounded outline-none py-2 pl-3 bg-white"
                onChange={e => setInputData({ ...inputData, GiaTri: e.target.value })}
              />
            </div>
            <br />
            <div className="flex justify-around ">
              ...
              <button onClick={() => navigat("../rules")} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-[10px]">
                Thoát
              </button>
            </div>
          </form>
