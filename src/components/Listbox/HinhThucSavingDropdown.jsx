...
import { setPhuongThuc } from '../../redux/customer/depositSaving/customerDepositSavingSlice'
...
export default function ConfirmationDropdown({ }) {
    ...
    const options = [
        { name: 'Lãi nhập gốc' },
        { name: 'Lãi trả vào tài khoản thanh toán' },
    ]

    const [selected, setSelected] = useState(PhuongThuc);

    useEffect(() => {
        setSelected(selected);
        dispatch(setPhuongThuc(selected))
    }, [selected]);
                    ...
                    <Listbox.Button className="relative w-full cursor-default rounded-[5px] bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                        {
                            selected ?
                                (
                                    <span className="block truncate text-xl text-[#7AC014] font-museo-slab-100">
                                        {selected.name}
                                    </span>
                                )
                                : (
                                    <span className="block truncate text-xl text-gray-400 ">
                                        Chọn hình thức trả lãi
                                    </span>
                                )
                        }
                                ...
                                >
                                    {({ selected }) => (
                                        <>
                                            ...
                                            {selected ? (
                                                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-[#7AC014]">
